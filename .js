...
// 1. ADD YOUR BOOKS HERE - NO CODING NEEDED AFTER THIS
const BOOKS_DATA = [
  {
    id: "book1",
    title: "Whispers in Lavender",
    blurb: "Two girls meet under a lavender tree and everything changes.",
    cover: "https://via.placeholder.com/150x220/c084fc/ffffff?text=Whispers", // book cover icon
    views: 0,
    chapters: [
      {
        title: "Chapter 1: First Meeting",
        // Option A: Direct text. Use | to split pages
        content: "Page 1 text here|Page 2 text here|Page 3 text here"
        // Option B: Google Docs link. Uncomment below and comment content above
        // content: "https://docs.google.com/document/d/YOUR_DOC_ID/export?format=txt",
        // isDocLink: true
      },
      {
...
async function openBook(key) {
  ...
  // Increment views - but NOT for admin
  if(!isAdmin) {
...
async function fetchDocContent(url){
  try {
    // Use a cors proxy because Google blocks direct fetch
    const res = await fetch(`https://api.allorigins.win/raw?url=${encodeURIComponent(url)}`);
    let text = await res.text();
    // Convert Google Docs ---PAGE BREAK--- to |
    return text.replace(/---PAGE BREAK---/g, '|');
  } catch(e) {
    return "Error loading Google Doc. Make sure it's 'Anyone with link can view' and published.";
  }
}
...
async function loadPage(){
  ...
  // If it's a Google Docs link, fetch it
  ...
  document.getElementById('pageContent').innerHTML = `<p>${pages[currentPage]}</p>`;
  document.getElementById('pageNum').textContent = `Page ${currentPage + 1} of ${pages.length}`;
  window.scrollTo(0,0);
}

function nextPage(){
  const pages = currentBook.chapters[currentChapter].content.split('|').length;
...
function prevPage(){
  ...
  else if(currentChapter > 0){ currentChapter--; currentPage = 9; } // will auto fix on load
  loadPage(); saveProgress();
}
